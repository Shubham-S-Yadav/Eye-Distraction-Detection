...
video_output = cv2.VideoWriter('captured_video.avi', fourcc, 10.0, (1200, 900))
        ...
        if len(faces) > 0:
            ...
            for (x, y, w, h) in faces:
                ...
                eyes = eye_cascade.detectMultiScale(roi_gray, scaleFactor=scale_factor, minNeighbors=min_neighbours,
                                                    minSize=(min_size_w_eye, min_size_w_eye))
                ...
                # get label
                if probs_mean <= Distraction_Threshold:
                    cnt += 1
                    if cnt > Frames_Threshold:
